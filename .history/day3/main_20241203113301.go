package main

import (
	"fmt"
	"io"
	"os"
	"regexp"
	"strconv"
	"strings"
)

func findMulOfMatches(result string) int {
	sum := 0
	regex2, _ := regexp.Compile(`mul\([0-9]+,[0-9]+\)`)
	matches := regex2.FindAllString(result, -1)
	for _, match := range matches {
		var numStr string
		var int1 int
		var int2 int
		var err error
		for i := 4; i < len(match); i++ {
			if match[i] == ',' {
				int1, err = strconv.Atoi(numStr)
				numStr = ""
				if err != nil {
					break
				}
				continue
			}
			if match[i] == ')' {
				int2, err = strconv.Atoi(numStr)
				break
			}
			numStr = numStr + string(match[i])
		}
		sum += (int1 * int2)
	}
	return sum
}
func main() {
//	regex1, _ := regexp.Compile(`do\(\)(.*?)don't\(\)`)
	//PART ONE
	// sum:=0
	// for {
	// 	stringData, errInput := r.ReadString('\n')
	//	sum+= findMulOfMatches(stringData)
	// 	matches := regex2.FindAllString(stringData,-1)
	// 	for _, match:= range matches{
	// 		var numStr string
	// 		var int1 int
	// 		var int2 int
	// 		fmt.Println(match)

	// 		for i:=4 ; i<len(match) ; i++{
	// 			if(match[i]==','){
	// 				int1, err= strconv.Atoi(numStr)
	// 				numStr=""
	// 				continue
	// 			}
	// 			if(match[i]==')'){
	// 				int2,err = strconv.Atoi(numStr)
	// 				break
	// 			}
	// 			numStr= numStr+ string(match[i])
	// 		}
	// 		sum+=(int1*int2)
	// 	}

	// 	if errInput != nil {
	// 		break // End of file or read error
	// 	}

	// }
	// fmt.Println(sum)
	//PART TWO
	wholeSum := 0
	file, err := os.Open("day3/data-input3.txt")
	if err != nil {
		return
	}
	defer file.Close()

	// Read the whole file
	data, err := io.ReadAll(file)
	if err != nil {
	}
	stringData := string(data)
	stopOccurance := strings.Index(stringData, "don't()")
	// startOccurance := strings.Index(stringData, "do()")
	// if stopOccurance > startOccurance {
	// 	stopOccurance=startOccurance
	// }
	startOccurance:=0
	wholeSum += findMulOfMatches(stringData[:stopOccurance])
	
	println(wholeSum)
	for{
		startOccurance=strings.Index(stringData[stopOccurance:],"do()")
		if startOccurance != -1 {
			startOccurance += stopOccurance  
		}
		stopOccurance=strings.Index(stringData[startOccurance:],"don't()")
		if stopOccurance != -1 {
			stopOccurance += stopOccurance  // Adjust index relative to full string
		}
		fmt.Println(startOccurance)
		fmt.Println("now stop")
		fmt.Println(stopOccurance)
		fmt.Println()
		if(startOccurance==-1 && stopOccurance==-1){
			break
		}
		if(stopOccurance==-1 && startOccurance!=-1){
			wholeSum+=findMulOfMatches(stringData[startOccurance:])
			break
		}
		wholeSum += findMulOfMatches(stringData[startOccurance:stopOccurance])
	}
	// result := regex1.FindAllString(stringData, -1)
	// for _, res := range result {
	// 	wholeSum += findMulOfMatches(res)
	// }
	// fmt.Println()
	// lastOccurence := strings.LastIndex(stringData, "do()")
	// lastOccurenceOfDont := strings.LastIndex(stringData, "don't()")
	// if lastOccurence > lastOccurenceOfDont {
	// 	fmt.Println(stringData[lastOccurence:])
	// 	wholeSum += findMulOfMatches(stringData[lastOccurence:])
	// }

	fmt.Println(wholeSum)
}
