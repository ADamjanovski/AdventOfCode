	...
	for i:=0; i<len(charsMatrix);i++{
		for j:=0; j<len(charsMatrix[i]);j++{
			if(charsMatrix[i][j]=="A" && i>=1 && j>=1 && (len(charsMatrix)-i)>1 && (len(charsMatrix[i])-j)>1){
				if((charsMatrix[i-1][j-1]==charsMatrix[i+1][j-1]=="M" && charsMatrix[i-1][j+1]==charsMatrix[i+1][j+1]=="S")||
				(charsMatrix[i-1][j-1]==charsMatrix[i+1][j-1]=="S" && charsMatrix[i-1][j+1]==charsMatrix[i+1][j+1]=="M")
			)
			}
		}
	}
